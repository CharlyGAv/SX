...
const SurveyScriptGenerator = ({ onBack }: SurveyScriptGeneratorProps) => {
  ...
  const [isSinAfectacionSelected, setIsSinAfectacionSelected] = useState(false);
  const [selectedComponentTab, setSelectedComponentTab] = useState("HW");
  ...
  const allComponentesHWOpciones = [
    "PC-FUENTE PODER",
    "SUC-PC-CPU",
    "LAPTOP-DISPLAY",
    "LAPTOP-TOUCHPAD",
    ...
    "SUC-TELEFONIA-RADIO",
    "SUC-TELEFONIA-TELEFONO",
    "SUC-TELEFONIA-DIADEMA",
    ...
    "APPLE-IPAD KARDEX",
    "HW - OTRO - DESCRIBE EN COMENTARIOS"
  ];

  const allComponentesSWOpciones = [
    ...
    "ACROBAT READER (PDF)",
    "DRIVER-ACCESORIOS",
    "SW - OTRO - DESCRIBE EN COMENTARIOS"
  ];

  const allComponentesRedOpciones = [
    "ROUTER",
    "LANSWITCH",
    "ACCESS POINT (WAP)",
    "MODEM WIFI",
    ...
    "ENERGÍA ELÉCTRICA",
    "MEDIO TELECOMM",
    "RED - OTRO - DESCRIBE EN COMENTARIOS"
  ];

  const allComponentesUsuarioOpciones = [
    "CMP REQUERIMIENTO",
    "PROBLEMAS CON PASSWORDS",
    "INFRAESTRUCTURA DEL EDIFICIO (INMUEBLES)",
    "USUARIO - OTRO - DESCRIBE EN COMENTARIOS"
  ];

  const componentesHWOpciones = script.tipoDeUsuario === "EDIFICIO/OFICINAS" ? [
    "DEP-PC-CPU",
    "PC-FUENTE PODER",
    "LAPTOP-DISPLAY",
    "LAPTOP-TOUCHPAD",
  ...
  ] : script.tipoDeUsuario === "SUCURSAL BACKOFFICE" ? [
    "PC-FUENTE PODER",
    "SUC-PC-CPU",
    "ACC-TECLADO",
    "ACC-MOUSE",
    ...
    "APPLE-IPAD KARDEX",
    "OTRO - DESCRIBE EN COMENTARIOS"
  ] : script.tipoDeUsuario === "EJECUTIVO MOVIL" ? [
    "LAPTOP-DISPLAY",
    "LAPTOP-TOUCHPAD",
    "LAPTOP-TECLADO",
    "LAPTOP-CARGADOR",
    ...
    "IMPRESORA LASER",
    "OTRO - DESCRIBE EN COMENTARIOS"
  ] : (script.tipoDeUsuario === "SUCURSAL EJECUTIVO" || script.tipoDeUsuario === "SUCURSAL WORKBENCH" || script.tipoDeUsuario === "SUCURSAL CAJAS") ?
    allComponentesHWOpciones.filter(opcion =>
      !opcion.startsWith("LAPTOP") &&
      !opcion.startsWith("QMS") &&
      !opcion.startsWith("DEP")
    ).map(opcion => opcion === "PC-CPU" ? "SUC-PC-CPU" : opcion) : script.tipoDeUsuario === "SUCURSAL QMS" ?
    allComponentesHWOpciones.filter(opcion => opcion.startsWith("QMS")).concat("SUC-PC-CPU").filter((value, index, self) => self.indexOf(value) === index) : allComponentesHWOpciones;

  const componentesSWOpciones = script.tipoDeUsuario === "EDIFICIO/OFICINAS" ? 
    allComponentesSWOpciones.filter(opcion => opcion !== "TEOS") : script.tipoDeUsuario === "SUCURSAL QMS" ?
    ["QMS APLICATIVO"] : allComponentesSWOpciones;

  const componentesRedOpciones = script.tipoDeUsuario === "EDIFICIO/OFICINAS" ? 
    allComponentesRedOpciones : allComponentesRedOpciones;

  const componentesUsuarioOpciones = script.tipoDeUsuario === "EDIFICIO/OFICINAS" ? 
    allComponentesUsuarioOpciones.filter(opcion => opcion !== "SIN FALLA") : allComponentesUsuarioOpciones;
  ...
  // Function to clean component names by removing prefixes
  const cleanComponentName = (text: string): string => {
    // Remove common prefixes
    const prefixesToRemove = [
      'HW - ', 'SW - ', 'RED - ', 'Usuario - ', 'USUARIO - ',
      'ACC-', 'SUC-', 'DEP-', 'QMS-', 'LAPTOP-', 'PC-',
      'SUCURSAL-', 'TEL-','DEP-ACC-','SUC-TELEFONIA-',
    ];
  ...
  // Function to generate automatic comment
  const generateAutoComment = (tipoDeSolucion: string, componente: string): string => {
    const cleanedAction = cleanComponentName(tipoDeSolucion);
    const cleanedComponent = cleanComponentName(componente);
    return `SE REALIZA ${cleanedAction} DE ${cleanedComponent}`;
  };
  ...
  const getConfigurationItem = (componente: string): string => {
    switch (componente) {
      ...
      case "ACC-MOUSE":
      case "ACC-MONITOR":
      case "ACC-CABLE HDMI/DP":
      case "ACC-HUB USB":
      case "ACC-DOCKING STATION":
        if (script.tipoDeUsuario === "EDIFICIO/OFICINAS") {
      ...
      case "LAPTOP-CPU":
      case "DEP-ACC-HUB USB":
      case "DEP-ACC-DOCKING STATION":
      case "DEP-ACC-TECLADO":
      case "DEP-ACC-MOUSE":
      case "DEP-ACC-MONITOR":
        return "PHYSICAL DESKTOP SERVICES-LATAM-PROD-TSO";
      ...
      case "SUC-TELEFONIA-RADIO":
      case "SUC-TELEFONIA-TELEFONO":
      case "SUC-TELEFONIA-DIADEMA":
      ...
      case "HW - OTRO - DESCRIBE EN COMENTARIOS":
        return "NO EXISTE"; // O el CI que corresponda para HW
      case "SW - OTRO - DESCRIBE EN COMENTARIOS":
        return "NO EXISTE"; // O el CI que corresponda para SW
      case "RED - OTRO - DESCRIBE EN COMENTARIOS":
        return "NO EXISTE"; // O el CI que corresponda para Red
      case "USUARIO - OTRO - DESCRIBE EN COMENTARIOS":
        return "NO EXISTE"; // O el CI que corresponda para Usuario
      default:
  ...
  const handleChange = (field: keyof ScriptData, value: string) => {
    setScript(prevScript => {
      let updatedScript = { ...prevScript, [field]: value };

      if (field === 'numeroDeIncidente') {
        const digitsOnly = value.replace(/\D/g, '').slice(0, 10);
        updatedScript.numeroDeIncidente = digitsOnly;
      }

      if (field === 'componente') {
        const tipoSoporte = getTipoDeSoporte(value);
        updatedScript.tipoDeSoporte = tipoSoporte;
        updatedScript.configurationItem = getConfigurationItem(value);

        // Auto-switch to the correct tab based on component type
        setSelectedComponentTab(tipoSoporte);

        // Generate auto comment if action is already selected
        if (prevScript.tipoDeSolucion && prevScript.tipoDeSolucion !== '') {
          const currentTipoDeSoporte = tipoSoporte as keyof typeof tipoDeSolucionOpciones;
          const lastOption = tipoDeSolucionOpciones[currentTipoDeSoporte]?.[tipoDeSolucionOpciones[currentTipoDeSoporte].length - 1];

          if (prevScript.tipoDeSolucion === lastOption) {
            updatedScript.comentarios = "SE VALIDA USUARIO SIN AFECTACIÓN, SE OBTIENE VOBO PARA PROCEDER CON SU CIERRE.";
          } else {
            updatedScript.comentarios = generateAutoComment(prevScript.tipoDeSolucion, value);
          }
        }
      }

      if (field === 'tipoDeSoporte') {
        updatedScript.tipoDeSolucion = '';
        updatedScript.componente = '';
        updatedScript.configurationItem = 'NO EXISTE';
      }

      if (field === 'solucionEnTiempo' && value === "Sí") {
        updatedScript.motivoRetraso = '';
      }

      if (field === "afectacionUsuario") {
        const sinAfectacion = value === "SIN AFECTACION";
        setIsSinAfectacionSelected(sinAfectacion);
        if (sinAfectacion) {
          // Clear previous selections but don't auto-select the last option yet
          updatedScript.tipoDeSolucion = '';
          updatedScript.comentarios = '';
        } else {
          // If "SIN AFECTACION" is deselected, clear the auto-filled fields if they match the auto-fill values
          if (prevScript.comentarios === "SE VALIDA USUARIO SIN AFECTACIÓN, SE OBTIENE VOBO DE CIERRE.") {
            updatedScript.tipoDeSolucion = '';
            updatedScript.comentarios = '';
          }
        }
      ...
      if (field === "tipoDeSolucion") {
        const currentTipoDeSoporte = updatedScript.tipoDeSoporte as keyof typeof tipoDeSolucionOpciones;
        const lastOption = tipoDeSolucionOpciones[currentTipoDeSoporte]?.[tipoDeSolucionOpciones[currentTipoDeSoporte].length - 1];

        // If user selects the last option of any category, auto-fill comments
        if (value === lastOption) {
          updatedScript.comentarios = "SE VALIDA USUARIO SIN AFECTACIÓN, SE OBTIENE VOBO PARA PROCEDER CON SU CIERRE.";
        } else if (prevScript.comentarios === "SE VALIDA USUARIO SIN AFECTACIÓN, SE OBTIENE VOBO PARA PROCEDER CON SU CIERRE.") {
          // If the user changes from the auto-filled state, clear comments
          updatedScript.comentarios = '';
        } else if (updatedScript.componente && updatedScript.componente !== '') {
          // Generate auto comment when both component and action are selected
          updatedScript.comentarios = generateAutoComment(value, updatedScript.componente);
        }
      }

      // Special handling for comments to prevent deletion of auto-generated text
      if (field === 'comentarios') {
        const autoGeneratedComment = generateAutoComment(prevScript.tipoDeSolucion, prevScript.componente);
        const isSinAfectacionComment = "SE VALIDA USUARIO SIN AFECTACIÓN, SE OBTIENE VOBO PARA PROCEDER CON SU CIERRE.";

        if (prevScript.comentarios === isSinAfectacionComment) {
          // If the previous comment was the "SIN AFECTACION" auto-comment, it remains read-only.
          // Any attempt to change it will revert to the original.
          updatedScript.comentarios = isSinAfectacionComment;
        } else if (autoGeneratedComment && prevScript.comentarios.startsWith(autoGeneratedComment)) {
          // If there's an auto-generated comment (not "SIN AFECTACION") and user is trying to modify
          // Ensure the auto-generated part stays at the beginning
          if (!value.startsWith(autoGeneratedComment)) {
            // If user deleted part of auto-generated text, restore it and add their text
            const userAddedText = value.substring(autoGeneratedComment.length);
            updatedScript.comentarios = autoGeneratedComment + userAddedText;
          } else {
            // User is adding text after the auto-generated part or just typing
            updatedScript.comentarios = value;
          }
        } else {
          // For any other case, allow normal editing
          updatedScript.comentarios = value;
        }
      }
  ...
  const resetForm = () => {
    ...
    setShowFinalScript(false);
    setCopied(false);
    setSelectedComponentTab("HW");
    setIsSinAfectacionSelected(false);
  };
    ...
    return (
      <div className="space-y-8 text-center">
        <div className="bg-gradient-to-r from-orange-500 to-orange-600 text-white p-6 rounded-lg shadow-lg">
          ...
          <p className="text-orange-100">Revisa y copia el script generado</p>
        </div>
        ...
        return (
          <div className="space-y-8 text-center">
            <div className="bg-gradient-to-r from-orange-500 to-orange-600 text-white p-6 rounded-lg shadow-lg">
              ...
              <p className="text-orange-100">Ingresa los 10 dígitos del número de incidente (se agregará automáticamente el prefijo "INC")</p>
            </div>
                ...
                <input
                  className={`w-full pl-12 pr-4 py-3 bg-white border ${
                    script.numeroDeIncidente && script.numeroDeIncidente.length !== 10
                      ? 'border-red-500'
                      : 'border-gray-300'
                  } rounded-lg text-gray-800 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent text-lg font-mono`}
                  type="text"
        ...
        return (
          <div className="space-y-8 text-center">
            <div className="bg-gradient-to-r from-orange-500 to-orange-600 text-white p-6 rounded-lg shadow-lg">
              ...
              <p className="text-orange-100">Selecciona el tipo de usuario que reportó el incidente</p>
            </div>
        ...
        return (
          <div className="space-y-8 text-center">
            <div className="bg-gradient-to-r from-orange-500 to-orange-600 text-white p-6 rounded-lg shadow-lg">
              ...
              <p className="text-orange-100">¿Qué tan afectado se vio el usuario por este incidente?</p>
            </div>
        ...
        return (
          <div className="space-y-8 text-center">
            <div className="bg-gradient-to-r from-orange-500 to-orange-600 text-white p-6 rounded-lg shadow-lg">
              ...
              <p className="text-orange-100">¿Qué fue lo que reportó el usuario o qué solicitud estás atendiendo?</p>
            </div>

            <div className="max-w-4xl mx-auto">
              <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-200">
                <Tabs value={selectedComponentTab} onValueChange={setSelectedComponentTab} className="w-full">
                  <TabsList className="grid w-full grid-cols-4 mb-6">
                    <TabsTrigger value="HW" className="font-semibold">Hardware</TabsTrigger>
                    <TabsTrigger value="SW" className="font-semibold">Software</TabsTrigger>
                    <TabsTrigger value="RED" className="font-semibold">Red</TabsTrigger>
                    <TabsTrigger value="USUARIO" className="font-semibold">Usuario</TabsTrigger>
                  </TabsList>

                  <TabsContent value="HW" className="mt-0">
        ...
        return (
          <div className="space-y-8 text-center">
            <div className="bg-gradient-to-r from-orange-500 to-orange-600 text-white p-6 rounded-lg shadow-lg">
              ...
              <p className="text-orange-100">¿Qué acción tomaste para resolver el incidente?</p>
            </div>

            <div className="grid grid-cols-1 gap-4 max-w-md mx-auto">
              {(script.tipoDeSoporte ? tipoDeSolucionOpciones[script.tipoDeSoporte as keyof typeof tipoDeSolucionOpciones] : [])?.map((option, index, array) => {
                const isLastOption = index === array.length - 1;
                const isSinAfectacionSelected = script.afectacionUsuario === "SIN AFECTACION";
                const isDisabled = isSinAfectacionSelected && !isLastOption;
                ...
                return (
                  <button
                    ...
                    disabled={isDisabled}
                    className={`px-6 py-4 rounded-xl text-lg font-medium transition-all ${
                      script.tipoDeSolucion === option
                        ? 'bg-orange-500 text-white shadow-md'
                        : isDisabled
                          ? 'bg-gray-200 text-gray-400 cursor-not-allowed border border-gray-300'
                          : 'bg-white text-gray-800 hover:bg-gray-100 border border-gray-200'
                    }`}
                    onClick={() => !isDisabled && handleChange('tipoDeSolucion', option)}
                  >
                    {option}
                  </button>
                );
        ...
        return (
          <div className="space-y-8 text-center">
            <div className="bg-gradient-to-r from-orange-500 to-orange-600 text-white p-6 rounded-lg shadow-lg">
              ...
              <p className="text-orange-100">Este campo se autocompleta según el componente seleccionado</p>
            </div>
        ...
        return (
          <div className="space-y-8 text-center">
            <div className="bg-gradient-to-r from-orange-500 to-orange-600 text-white p-6 rounded-lg shadow-lg">
              ...
              <p className="text-orange-100">¿Se resolvió el incidente dentro del tiempo esperado?</p>
            </div>
        ...
        return script.solucionEnTiempo === "No" ? (
          <div className="space-y-8 text-center">
            <div className="bg-gradient-to-r from-orange-500 to-orange-600 text-white p-6 rounded-lg shadow-lg">
              ...
              <p className="text-orange-100">¿Cuál fue el motivo por el que no se resolvió a tiempo?</p>
            </div>
          ...
          <div className="mb-4 flex flex-col items-center">
            <label className="block text-gray-700 text-sm font-bold mb-2">Comentarios Finales:</label>
            <textarea
              className={`shadow-md appearance-none border border-gray-300 rounded-lg w-full max-w-md py-3 px-4 leading-tight focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent transition duration-200 ease-in-out resize-none ${
                script.comentarios === "SE VALIDA USUARIO SIN AFECTACIÓN, SE OBTIENE VOBO PARA PROCEDER CON SU CIERRE." 
              ...
              readOnly={script.comentarios === "SE VALIDA USUARIO SIN AFECTACIÓN, SE OBTIENE VOBO PARA PROCEDER CON SU CIERRE."}
            ></textarea>
          </div>
        );
      case 9:
        return (
          <div className="space-y-8 text-center">
            <div className="bg-gradient-to-r from-orange-500 to-orange-600 text-white p-6 rounded-lg shadow-lg">
              ...
              <p className="text-orange-100">Agrega cualquier información adicional relevante</p>
            </div>

            <div className="max-w-md mx-auto">
              <textarea
                className={`shadow-md appearance-none border border-gray-300 rounded-lg w-full py-3 px-4 leading-tight focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent transition duration-200 ease-in-out resize-none ${
                  script.comentarios === "SE VALIDA USUARIO SIN AFECTACIÓN, SE OBTIENE VOBO PARA PROCEDER CON SU CIERRE." 
                    ? 'bg-gray-100 text-gray-600 cursor-not-allowed' 
                    : 'text-gray-700'
                }`}
              ...
              {script.comentarios === "SE VALIDA USUARIO SIN AFECTACIÓN, SE OBTIENE VOBO PARA PROCEDER CON SU CIERRE." && (
                <p className="text-sm text-gray-500 mt-2">
                  Campo completado automáticamente - Solo lectura
                </p>
              )}
              {script.comentarios.startsWith("Se realiza ") && (
                <p className="text-sm text-orange-600 mt-2">
                  Texto protegido contra borrado - Puedes agregar contenido adicional
                </p>
              )}
            </div>
          </div>
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-blue-50 py-8 px-4">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-1xl font-bold text-gray-800">Generador de Scripts de Cierre</h1>
          <p className="text-gray-500 mt-2">Completa la información paso a paso para generar tu script de cierre</p>
        </div>
        ...
        <div className="bg-white rounded-3xl shadow-2xl overflow-hidden">
          ...
          {/* Navigation */}
          <div className="bg-gray-50 px-8 py-6 flex justify-center items-center">
            <div className="flex justify-between items-center w-full max-w-md">
              <button
                onClick={prevStep}
                className={`flex items-center gap-2 px-6 py-3 text-gray-600 hover:text-gray-800 transition-colors rounded-xl
                  ${step === 1 ? 'invisible' : ''}
                `}
                disabled={step === 1}
              >
        ...
        <div className="text-center text-gray-400 text-xs mt-8">
          Administrado por el Área de Calidad y Procesos.
        </div>
      </div>
    </div>
  );
